...
| `v0.1.4` | ✅ 当前版本 | 架构级修复：桌面壳窗口直接加载 `http://127.0.0.1:8791`，与浏览器同源，彻底消除 `tauri://` 跨源问题。 |

> **一句话记住**：桌面壳**不含后端**。它只是一个窗口。后端没起，桌面端就是空白页 —— 这是设计，不是故障。见 §2.4。
...
> ⚠️ **桌面壳不含后端。** 它只是一个窗口，加载 `http://127.0.0.1:8791`
> （后端同源托管的前端）。**后端没启动，桌面端就是空白页** —— 这是设计，不是故障。
...
```text
┌── Tauri 桌面壳（无业务逻辑）──┐        ┌── 浏览器（等价入口）──┐
│  窗口直接加载                  │        │  打开同一地址         │
│  http://127.0.0.1:8791         │        │                       │
└───────────────┬────────────────┘        └───────────┬───────────┘
                └───────────────┬─────────────────────┘
                                ▼  同源（无 CORS、无 tauri:// 协议问题）
┌─────────────────────────────────────────────────────────────────────────────────────┐
...
| # | 项 | 现状 |
|---|---|---|
| 1 | **桌面壳不含 Python 后端** | 安装包只含前端静态资源。**必须自己先起后端**（`启动 OpsPilot.bat` 或 `uvicorn ... --port 8791`），否则桌面端是空白页。未做 PyInstaller sidecar。 |
...
| **P0-6** | v0.1.1 / v0.1.3 实机仍连不上（`isTauri()` 与多候选探测均未命中，`tauri://` 下 fetch 根本没发出） | **架构级修复 v0.1.4**：`tauri.conf.json` 的 `windows[].url` 直接指向 `http://127.0.0.1:8791`，后端 `app.mount("/", StaticFiles)` 同源托管前端 → 无跨源、无 `tauri://` 协议问题 |
...
| v0.1.4 | 改架构：窗口 URL 直接指向 `http://127.0.0.1:8791` | —（从源头消除跨源） |

**教训（写进流程）**

1. **不要靠"看起来正常"判定成功。** 唯一可靠证据是 `netstat -ano | findstr :8791` 出现 **`ESTABLISHED`**，以及后端 uvicorn 访问日志有新请求。
2. **改了前端代码，要能在产物二进制里搜到特征串**；搜不到说明没打进去或没执行到。
3. **能靠架构消除的问题，不要用运行时兼容代码去绕。**
